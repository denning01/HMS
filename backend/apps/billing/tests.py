...
def test_seed_services_creates_the_starting_catalogue(services):
    assert Service.objects.count() == 13
    assert Service.objects.filter(department=Department.LABORATORY).count() == 5
...
def test_retired_services_stay_in_the_catalogue(services):
    ...
    assert Service.objects.filter(is_active=True).count() == 12
