...
def test_collections_shows_todays_takings(cashier, client):
    pay_for(make_visit(), ["CONS-GEN"], cashier)
    client.force_login(cashier)

    body = client.get(reverse("collections")).content.decode()

    assert "500.00" in body
    assert "Caleb Omondi" in body
    assert "RCT" in body


def test_collections_can_look_at_an_earlier_day(cashier, client):
    payment = pay_for(make_visit(), ["CONS-GEN"], cashier)
    ...
    body = client.get(reverse("collections"), {"day": yesterday.isoformat()}).content.decode()
...
def test_an_unparseable_date_falls_back_to_today(cashier, client):
    ...
    response = client.get(reverse("collections"), {"day": "not-a-date"})

    assert response.status_code == 200
    assert b"not a date" in response.content


@pytest.mark.parametrize("role", [Role.TRIAGE_NURSE, Role.PHARMACIST, Role.RECEPTIONIST])
def test_collections_is_closed_to_clinical_roles(roles, client, role):
    client.force_login(make_user(f"user-{role.value}", role))

    assert client.get(reverse("collections")).status_code == 403


def test_the_finance_manager_can_read_collections(roles, client):
    client.force_login(make_user("finance", Role.FINANCE_MANAGER))

    assert client.get(reverse("collections")).status_code == 200
